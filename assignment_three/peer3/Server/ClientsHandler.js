   ...
   sock.on('data', (data) => {
      ...
      //cHECK LOCAL Keys to see if the IMAGE is in the lkl, if we cannot find the image try to send query
        ...
        if (getSize(singleton.getDHT()) != 0) { //check if dht
          ...
          console.log(getclosestPeer(file))
          let v= getclosestPeer(file)
          ip = String(v[0])
          p = Number(v[1])
...
function getclosestPeer(rq) {
  ...
  let closest;
  dht.forEach(element => {
    if (singleton.getLeft(singleton.Hex2Bin(element[2]),key) > max && (singleton.getID() != element[2]) && (element[1] != singleton.getServer())) {
      closest = element
    }
  });
