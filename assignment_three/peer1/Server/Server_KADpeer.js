...
module.exports = {
    ...
    runServer: function(hostName) {
        ...
        imageDB.listen(PORTA, HOST);

        console.log('ImageDB server is started at timestamp: '+singleton.getTimestamp()+' and is listening on ' + HOST + ':' + PORTA);

        imageDB.on('connection', function(sock) {
            handler.handleImageClientJoining(sock); //called for each client joining
});
